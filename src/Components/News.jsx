...
export default class News extends Component {
  ...
  handleNextClick= async ()=>{
    ...
    let url=`https://newsapi.org/v2/top-headlines?country=us&apiKey=b4e92df4ad63422e820ec3184968894f&page=1${this.state.page +1}&pageSize=20`;
    let data = await fetch(url);
    let parsedData = await data.json();
    console.log(parsedData);

this.setState({
  page: this.state.page +1, article:parsedData.articles})
}
  }
...
<NewsItem  className="card-img-top"  title={element.title?element.title.slice(0,45):""} description={element.description?element.description.slice(0,88):""} imageUrl={element.urlToImage?element.urlToImage:"https://www.shutterstock.com/image-photo/global-business-structure-networking-analysis-260nw-2151657341.jpg"} newUrl={element.url}/>
</div>
...
<button className="btn btn-success" onClick={this.handleNextClick}>Next &rarr;</button>
